...
public class VentaController extends TiendaApp {
	...
	@PostMapping(value = "/ventas/items/update")
	public String updateVentaItem(@ModelAttribute("item") VentaItemCreateRequest datosVentaItem) {
		LOGGER.info("PUT - updateVentaItem - ventas/item/update");
		LOGGER.info("datosVentaItem: " + datosVentaItem.toString());
	    ...
	    Venta venta = null;
        try {
        	venta = ventaService.updateItem(datosVentaItem.getVentaId(), datosVentaItem.getPrendaId(), item);
        } catch (Exception e) {
            e.printStackTrace();
        }
		LOGGER.info("Venta Grabada: " + venta.toString());
	...
	@RequestMapping(value = "/ventas/{ventaId}/items/{itemId}", method = RequestMethod.GET)
	public ModelAndView showEditVentaPage(@PathVariable("ventaId") long ventaId,
			@PathVariable("itemId") long itemId) {
		LOGGER.info("GET - showEditVentaPage - /ventas/edit/{id}");
		LOGGER.info("venta: " + ventaId);

		ModelAndView mav = new ModelAndView("ventas/edit_item_venta");
		
		VentaItemCreateRequest vicr = new VentaItemCreateRequest();
        
		vicr.setVentaId(ventaId);
		vicr.setPrendaId(itemId);
		
		
		try {
			Venta venta = ventaService.findById(ventaId);
			mav.addObject("venta", venta);
			LOGGER.info("LA venta esta" + venta.toString());
		} catch (BusinessException e) {
			LOGGER.error("ERROR AL TRAER LA VENTA");
			
			e.printStackTrace();
		}
		try {
			Item item = itemService.findById(itemId);
			vicr.setCantidad(item.getCantidad());
			mav.addObject("item", item);
			mav.addObject("vicr", vicr);
		} catch (BusinessException e) {
